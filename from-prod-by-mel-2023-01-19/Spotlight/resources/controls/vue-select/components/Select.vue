...
<template>
  <div :dir="dir" class="v-select" :class="stateClasses">
    ...
    <transition :name="transition">
      <ul ref="dropdownMenu" v-if="dropdownOpen" :id="`vs${uid}__listbox`" class="vs__dropdown-menu ps-child" role="listbox" @mousedown.prevent="onMousedown" @mouseup="onMouseUp" >
        <slot name="list-header" v-bind="scope.listHeader" />

        <li role="option"
            v-for="option in _computeItems(filteredOptions, true)"
           :key="getOptionKey(option)"
           :id="`vs${uid}__option-${getOptionKey(option)}`"
            class="vs__dropdown-option priority"
           :class="{ 'vs__dropdown-option--selected': isOptionSelected(option), 'vs__dropdown-option--highlight': getOptionKey(option) === typeAheadPointer, 'vs__dropdown-option--disabled': !selectable(option) }"
           :aria-selected="getOptionKey(option) === typeAheadPointer ? true : null"
           @mouseover="selectable(option) ? typeAheadPointer = getOptionKey(option) : null"
        ...
        <li v-if="_computeItems(filteredOptions, true).length > 0" class="dropdown-divider vs__dropdown-option--disabled"></li>

        <li role="option"
            v-for="option in _computeItems(filteredOptions, false)"
           :key="getOptionKey(option)"
           :id="`vs${uid}__option-${getOptionKey(option)}`"
  ...
  export default {
    ...
    props: {
      ...
      getOptionKey: {
        ...
        default (option) {
          ...
          try {
            return option.hasOwnProperty(this.idProperty) ? option[this.idProperty] : sortAndStringify(option);
          } catch (e) {
    ...
    watch: {
      ...
      options (newOptions, oldOptions) {
        ...
        if(!this.value && newOptions && newOptions.length > 0 && this.autoSelect) {
          this.select(newOptions[0]);
        }
      },
    ...
    methods: {
      ...
      onSearchFocus() {
        ...
        setTimeout(function() {
          this._handleFindSelectedItem(null);
        }.bind(this), 50);
      },

      _handleFindSelectedItem: function(e) {
          let tt = $(this.$refs.dropdownMenu).find('li.vs__dropdown-option--selected');

          if(!tt || tt.length == 0) {
             tt = $(this.$refs.dropdownMenu).find('li.vs__dropdown-option');
          }
          ...
          tt[0].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' });
      },


      _handleFindHighlightedItem: function(e) {
          let tt = $(this.$refs.dropdownMenu).find('.vs__dropdown-option--highlight');
          if(!tt || tt.length == 0) {
              return;
          }
